...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # create somewhere to store the possible ghost positions
        allPossible = util.Counter()

        # Edge case: Ghost has been captured:
        if noisyDistance is None:
            # the ghost is in its jail cell
            allPossible[self.getJailPosition()] = 1.0
            self.beliefs = allPossible
            return
        
        # For each possible postion the ghost can be
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # Belief(current_state) = probability(emission | previous_state) * Belief'(current_state)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # For each state we belive a ghost may be at
        for oldPos in self.beliefs:
            # get the probability distribution of its possible moves
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))

            # Belief'(current_state) = sum(probability(possible moves)) * Belief(previous_state)
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.beliefs = []
        remaning_particles = self.numParticles

        # a cycle to get the locations for our particles
        position_iterator = itertools.cycle(self.legalPositions)

        # for each particle, assign it to the next legal position
        while remaning_particles > 0:
            self.beliefs.append(next(position_iterator))
            remaning_particles -= 1
    ...
    def observe(self, observation, gameState):
        ...
        # Edge case: Ghost has been captured:
        if noisyDistance is None:
            self.beliefs = [self.getJailPosition()] * self.numParticles
            return
        ...
        for p in self.beliefs:
            # calculate the probability(observation | particle)
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            weight = emissionModel[trueDistance]

            # add that weight to the total weight distribution
            weights[p] += weight
            
        # Edge case: all particles have weight 0:
        ...
        for _ in range(self.numParticles):
            new_beliefs.append(util.sample(weights))
    ...
    def elapseTime(self, gameState):
        ...
        # for each particle
        for p in self.beliefs:
            # calculate a distribution of it's movements
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))

            # pick one of those moves to move it to
            future_beliefs.append(util.sample(newPosDist))

        self.beliefs = future_beliefs
    ...
    def getBeliefDistribution(self):
        ...
        for p in set(self.beliefs):
            # Belief^ (current_state) = # p in state / total # p
            belief_distribution[p] = self.beliefs.count(p) / float(self.numParticles)
