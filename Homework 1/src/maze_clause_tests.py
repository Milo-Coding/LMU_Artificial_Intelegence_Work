...
class MazeClauseTests(unittest.TestCase):
    ...
    # adding more tests
    def test_mazeclause_resolution13(self) -> None:  # check that resolve doesn't mutate clauses
        mc1 = MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), True), (("A", (2, 0)), False)])
        mc2 = MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), False), (("A", (2, 0)), False)])
        res = MazeClause.resolve(mc1, mc2)
        self.assertEqual(mc1, MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), True), (("A", (2, 0)), False)]))
        self.assertEqual(mc2, MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), False), (("A", (2, 0)), False)]))
        
    def test_mazeclause_inverse1(self) -> None:  # check that the inverse helper function is working
        mc = MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False)])
        mc_inv = mc.invert()
        self.assertFalse(mc_inv.get_prop(("A", (1, 0))))
        self.assertTrue(mc_inv.get_prop(("A", (0, 0))))
        self.assertFalse(mc_inv.is_valid())
        self.assertFalse(mc_inv.get_prop(("A", (1, 0))))
        self.assertTrue(mc_inv.get_prop(("A", (0, 0))))
        self.assertFalse(mc_inv.is_valid())

    def test_mazeclause_inverse2(self) -> None: # check that valid clauses don't break it
        mc1 = MazeClause([(("X", (1, 1)), True)])
        mc2 = MazeClause([(("X", (1, 1)), False)])
        res = MazeClause.resolve(mc1, mc2)
        self.assertTrue(MazeClause([]).invert() in res)

    def test_mazeclause_inverse3(self) -> None:  # check larger clauses
        mc1 = MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), True), (("A", (2, 0)), False)])
        mc2 = MazeClause([(("A", (1, 0)), True), (("A", (0, 0)), False), (("A", (1, 4)), False), (("A", (2, 0)), False)])
        mc1_inv = mc1.invert()
        mc2_inv = mc2.invert()
        res = MazeClause.resolve(mc1_inv, mc2_inv)
        self.assertEqual(1, len(res))
        self.assertTrue(MazeClause([(("A", (1, 0)), False), (("A", (0, 0)), True), (("A", (2, 0)), True)]) in res)
